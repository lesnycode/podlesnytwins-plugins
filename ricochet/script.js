...
if (dialog && form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.status === 503) {
        throw new Error('Приём оплаты ещё настраивается. Напишите на podlesnytwins@gmail.com — вышлем счёт вручную.');
      }
      if (!res.ok) {
        throw new Error('Банк не принял заявку на оплату. Попробуйте ещё раз или напишите на podlesnytwins@gmail.com.');
      }

      const data = await res.json();
      if (!data.url) throw new Error('Сервис оплаты не вернул ссылку. Напишите на podlesnytwins@gmail.com.');
