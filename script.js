...
if (dialog && form) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status === 503 ? 'Оплата временно недоступна. Напишите нам на podlesnytwinsplugins@gmail.com.' : 'Не удалось создать платёж. Попробуйте ещё раз.');
      const data = await response.json();
      if (!data.url) throw new Error('Сервис оплаты не вернул ссылку. Напишите нам на podlesnytwinsplugins@gmail.com.');
